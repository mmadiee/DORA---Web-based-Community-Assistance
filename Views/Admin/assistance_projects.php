...
<head>
    ...
    <title>Assistance Projects</title>
...
$sql = "SELECT *, 
        CONCAT(tbl_sw_accs.fname, ' ', tbl_sw_accs.lname) AS organizer_name,
        tbl_assistance.image AS project_image
    FROM tbl_assistance
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $sql2 = "SELECT COUNT(DISTINCT appli_id) AS distinct_beneficiaries 
        FROM tbl_applicants 
        WHERE assistance_id = $id AND stat = 'Verified'";
        $distinct_beneficiaries_result = mysqli_query($conn, $sql2);
        $distinct_beneficiaries_row = mysqli_fetch_array($distinct_beneficiaries_result);
        $distinct_beneficiaries = $distinct_beneficiaries_row['distinct_beneficiaries'];

        // Progress bar calculation
        $percentage = ($distinct_beneficiaries / $row['avail_slot']) * 100;
        $remaining = $row['avail_slot'] - $distinct_beneficiaries;
        ...
        <div class="container w-75 content">
            <div class="row">
                <div class="col-lg-8">
                    <div class="proj-img">
                        <img src="../SocialWorker/assist_img/' . $row['project_image'] . '" style="width: 100%">
                    </div>
                    ...
                    <div class="proj-desc">
                        ...
                        <div class="goal">
                            <div class="row">
                                <div class="col-6">
                                    <div class="row"> <b>' . $row['avail_slot'] . '</b></div>
                                    <div class="row"> <p class="par"> Total Needed Beneficiaries </p></div>

                                    <div class="row"> <b>' . $row['deadline'] . '</b></div>
                                    <div class="row"> <p class="par"> Project Date </p></div>
                                </div>

                                <div class="col-6">';
        if ($distinct_beneficiaries >= $row['avail_slot']) {
            echo '
        ...
        echo '
                                    <div class="row"> <b>' . $distinct_beneficiaries . '</b></div>
                                    <div class="row"> <p class="par"> Beneficiaries Count </p></div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="col-4">
                    <div class="box">
                        <p class="progress-raised"> ' . $distinct_beneficiaries . ' registered beneficiaries</p>
        ...
        echo '
                        ...
                        <div class="row justify-content-center mt-3">
                            <div class="col-lg-8">
                                <canvas id="myPieChart-' . $row['assistance_id'] . '" style="max-width: 400px; margin: auto;"></canvas>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        echo '<script>
            var ctx' . $row['assistance_id'] . ' = document.getElementById("myPieChart-' . $row['assistance_id'] . '").getContext("2d");
            var myPieChart' . $row['assistance_id'] . ' = new Chart(ctx' . $row['assistance_id'] . ', {
                type: "pie",
                data: {
                    labels: ["Raised", "Remaining"],
                    datasets: [{
                        label: "Amount",
                        data: [' . $distinct_beneficiaries . ', ' . $remaining . '],
                        backgroundColor: [
                            "rgba(255, 99, 132, 0.6)",
                            "rgba(54, 162, 235, 0.6)"
                        ],
