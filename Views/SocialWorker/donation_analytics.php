...
<head>
    ...
    <style>
      ...
      footer{
        ...
        @media print {
            body {
                margin-top: -; /* Adjust the margin as needed */
            }
  ...
  <div class="sidebar close">
    ...
    <div class="row justify-content-center">
        <div class="col-md-12 text-center mb-3">
            <h3 id="panel_label">Donation Panel Options</h3>
        </div>
    </div>
    <div class="row justify-content-center">
    ...
    <div class="row justify-content-center">
        ...
        displayDonationData(null, null, $sw_id);  // Pass $sw_id as a parameter

        function displayDonationData($start_date, $end_date, $sw_id) {
            include 'includes/config.php';

            // PANEL 1
            $sql = "SELECT SUM(t.amount) AS total_amount, DATE_FORMAT(MAX(t.submitdate), '%M %d, %Y') AS latest_submitdate
                    FROM tbl_transaction t
                    JOIN tbl_don_proj dp ON t.don_project_id = dp.don_project_id
                    WHERE t.stat = 'Verified' AND dp.sw_id = $sw_id";

            if ($sw_id) {
                $sql .= " AND dp.sw_id = {$sw_id}"; 
            }
            ...
            if ($result->num_rows > 0) {
                ...
                if ($result_highest_amount->num_rows > 0) {
                    ...
                    $sql_latest_donation = "SELECT amount, DATE_FORMAT(submitdate, '%M %d, %Y') AS submitdate_formatted 
                                            ...
                                            JOIN tbl_don_proj dp ON t.don_project_id = dp.don_project_id
                                            WHERE t.stat = 'Verified' AND dp.sw_id = $sw_id
                                            ORDER BY transac_id DESC LIMIT 1";
                    if ($start_date && $end_date) {
...
<script type="text/javascript">
    ...
    function updateChart() {
        ...
        $.ajax({
            ...
            method: 'POST',
            data: { startDate: startDate, endDate: endDate, sw_id: sw_id },
            dataType: 'json',
...
<script>
    window.onload = function () {
        ...
        Plotly.newPlot('chartCanvas', data, layout, config);
                        ...
                        <!-- DONUT CHART -->
                        <div id="donorDonutChart-container" class="chart-container" style="text-align: center; width: 49%; border: 2px solid #000; margin: 20px 0 50px 0; padding: 5px;">
                            <h2 id="h2font" style="margin-top: 20px; color: darkblue;"><b>One-Time VS. Recurring Donors</h2></b>
                            <canvas id="donorDonutChart" style="max-height: 300px; width: 100%; margin-bottom: 20px;"></canvas>
                                </ul>
                            </div>
                        </div>
                        ...
                        <script>
                            var ctx = document.getElementById('donorDonutChart').getContext('2d');
