...
<body>
    ...
    <div class="container content">
        ...
        <div class="title">
            <h3>On-going Volunteering Projects</h3>
        </div>
        ...
        <div class="row">
            <div class="col-lg-8">
                ...
                $sql_base = "SELECT vp.*, 
                    ...
                    WHERE vp.vol_proj_id = v.vol_proj_id 
                    AND v.stat = 'Verified') AS vol_count 
                    FROM tbl_vol_proj vp
                    WHERE vp.proj_stat = 'ON GOING'";

                // Handle search input
                if (isset($_GET['search']) && !empty($_GET['search'])) {
                    $search = mysqli_real_escape_string($conn, $_GET['search']);
                    $sql_base .= " AND (vp.title LIKE '%$search%' OR vp.category LIKE '%$search%')";
                }
                ...
                if (isset($_GET['category']) && is_array($_GET['category']) && !empty($_GET['category'])) {
                    ...
                    $sql_base .= " AND vp.category IN ('$categories_str')";
                }
                ...
                switch ($sort) {
                    case 'closest':
                        $sql_base .= " ORDER BY 
                        (vol_count / vp.totalGoal) DESC,
                        vp.uploadDate DESC";
                        break;
                    case 'nearest':
                        $sql_base .= " ORDER BY vp.eventDate ASC";
                        break;
                    default:
                        $sql_base .= " ORDER BY vp.uploadDate DESC";
                        break;
                }
                ...
                if (mysqli_num_rows($req3) == 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($req3)) {
                        //PROGRESS BAR
                        $percentage = ($row['vol_count'] / $row['totalGoal']) * 100;
                        $remaining = $row['totalGoal'] - $row['vol_count'];
                            ...
                            <div class="card mb-3" style="max-width: 730px;">
                                <div class="row no-gutters position-relative">
                                    <div class="col-md-5 mb-md-0">
                                        <img class="card-img" src="\Views\SocialWorker\vol_img/' . $row['image'] . '">
                                    </div>
                                    <div class="col-md-7 position-static p-4 pl-md-0">
                                        <div class="card-body">
                                            ...
                                            <div class="box">
                                                <p class="progress-raised"> ' . $row['vol_count'] . ' registered volunteers</p>   
                                                <div class="progress" role="progressbar" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">
                                                    <div class="progress-bar" style="width: ' . $percentage . '%"></div>
                                                </div>';
                        if ($row['vol_count'] >= $row['totalGoal']) {
                            echo '<p class="progress-to-go">Goal Reached!</p>';
                        } else {
                            echo '
                                                    <p class="progress-to-go"> ' . $remaining . ' to go </p> ';
                        }
                        echo '
                                            </div>
                                            <a href="vol_projects.php?id=' . $row['vol_proj_id'] . '" class="stretched-link"></a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="search-box">
                    <form method="GET" action="">
                        ...
                        <div class="mb-3">
                            ...
                            $sql_categories = "SELECT DISTINCT category FROM tbl_vol_proj";
                        ...
                        <a href="volunteer.php" style="font-size: 14px; color: #777; font-style: italic; text-align: right;">Clear Filters</a><br><br>
                        ...
                        <!-- putting of description in the donations and volunteer <p class="description">'.$row['text'].'</p> -->
    
    <!-- INCLUDE STYLE FOR THE UPLOADED CONTENT -->
    <style><?php include 'Views\SocialWorker\css\vol-upload.css'; ?></style>
